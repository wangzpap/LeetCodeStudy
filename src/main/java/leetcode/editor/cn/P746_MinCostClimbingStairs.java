...
class Solution {
    ...
    public int minCostClimbingStairs(int[] cost) {
		...
		memory = new int[cost.length+1];
		Arrays.fill(memory,-1);
		return minH(cost.length);
    }
	...
	public int minH(int n){
		...
		int x  = Math.min(minH(n-1)+cost[n-1],minH(n-2)+cost[n-2]);
		memory[n] = x;
		return x;
	}
}
